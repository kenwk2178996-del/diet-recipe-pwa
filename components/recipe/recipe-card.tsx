...
function nut(r: RecipeCardData) {
  return Array.isArray(r.nutrition) ? r.nutrition[0] : r.nutrition;
}

function nutritionText(n: ReturnType<typeof nut>) {
  const parts = [];
  if (n?.kcal != null) parts.push(`${Math.round(n.kcal)}kcal`);
  if (n?.protein_g != null) parts.push(`P${Math.round(n.protein_g)}g`);
  return parts.length ? parts.join(" ・ ") : "栄養未算出";
}
      ...
      <Link href={`/recipes/${r.id}`}>
        <Card className="flex gap-3 overflow-hidden p-2">
          <div className="h-16 w-16 shrink-0 overflow-hidden rounded-lg bg-beige">
            {r.main_image_url
              ? <img src={r.main_image_url} alt="" className="h-full w-full object-cover" loading="lazy" />
          ...
          <div className="min-w-0 flex-1">
            ...
            <p className="mt-1 text-xs text-ink/60">
              {nutritionText(n)}{r.cook_time_min ? ` ・ ${r.cook_time_min}分` : ""}
            </p>
          </div>
        </Card>
      </Link>
    ...
    <Link href={`/recipes/${r.id}`}>
      <Card className="overflow-hidden">
        <div className="relative aspect-[4/3] bg-beige">
          {r.main_image_url
            ? <img src={r.main_image_url} alt="" className="h-full w-full object-cover" loading="lazy" />
            : <RecipeImageFallback title={r.title} />}
        ...
        <div className="p-2.5">
          <p className="truncate text-sm font-semibold">{r.title}</p>
          <p className="mt-0.5 text-xs text-ink/60">{nutritionText(n)}</p>
